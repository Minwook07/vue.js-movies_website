                    ...
                    <div class="swiper-slide bg-transparent" v-for="drama in dramaStore.drama_lists" :key="drama.id">
                        <div class="card-content position-relative rounded-3">
                            <RouterLink to="/">
                                ...
                                <div class="card-languages"
                                    :class="drama.language == 'Eng Sub' ? `bg-dager` : `bg-success`">
                                    {{ drama.language }}
                                </div>
                        ...
                        <div class="mt-2 text-start">
                            <RouterLink to="/ongoing" class="btn p-0 border-0 fw-medium"
                                :class="drama.status == 'Ongoing' ? `text-success` : `text-danger`">{{ drama.status
                                }}</RouterLink>
                            <br>
                            <RouterLink to="/drama_details"
                                class="text-start btn p-0 border-0 fw-medium text-secondary-kam">{{ drama.title
                                }}</RouterLink>
                        </div>
                    </div>
                </div>
